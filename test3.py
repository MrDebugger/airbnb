...
for j in range(20,24):
    ...
    for i in range(1,13):
        ...
        print("Month:",month)
        for off in range(0,20000,50):
            print("Page: ", (off//50)+1)
            oldIds = logs[:]
            for day in [["01","15"],["16","31"]]:
                params = (
                    ...
                    ('last_search_session_id', '7f269da3-35e4-4622-b211-852319978fa3'),
                    ('locale', 'fr'),
                    ('metadata_only', 'false'),
                    ...
                    ('version', '1.7.0'),
                    ('checkin', f'20{year}-{month}-{day[0]}'),
                    ('checkout', f'20{year}-{month}-{day[1]}'),
                )
                d = open('ids3.txt','a')
                while True:
                    ...
                    try:
                        for data in response.json()["explore_tabs"]:
                            for room in data["home_tab_metadata"]["remarketing_ids"]:
                                if room not in logs:
                                    logs.append(room)
                                    d.write(str(room)+'\n')
                        break
